...
impl<'a> RootMessageLabel<'a> {
    ...
    pub fn try_from(message: &MessageInfo) -> Option<Self> {
        ...
        with_ipc_buffer(|buffer| {
            ...
            match label {
                0x0 => Some(Self::RegisterIRQ(regs[0] as _)),
                0x1 => Some(Self::TranslateAddr(regs[0] as _)),
                0x2 => {
                    let bytes = buffer.msg_bytes();
                    Some(Self::FindService(unsafe {
                        core::str::from_raw_parts(bytes.as_ptr().add(REG_LEN), regs[0] as _)
                    }))
                }
                0x3 => Some(Self::AllocNotify()),
                _ => None,
            }
        })
    }
    ...
    pub fn to_label(&self) -> u64 {
        let n = match self {
            RootMessageLabel::RegisterIRQ(_) => 0,
            RootMessageLabel::TranslateAddr(_) => 1,
            RootMessageLabel::FindService(_) => 2,
            RootMessageLabel::AllocNotify() => 3,
        };
        Self::LABEL_START + n
    }
    ...
    pub fn build(&self) -> MessageInfo {
        ...
        with_ipc_buffer_mut(|buffer| {
            ...
            match self {
                RootMessageLabel::RegisterIRQ(irq_num) => {
                    regs[0] = *irq_num;
                    msg_size = 2;
                }
                ...
                RootMessageLabel::FindService(service_name) => {
                    regs[0] = service_name.len() as _;
                    buffer.msg_bytes_mut()[REG_LEN..REG_LEN + service_name.as_bytes().len()]
                        .copy_from_slice(service_name.as_bytes());
                    msg_size = 1 + reg_num(service_name.as_bytes().len());
                }
                RootMessageLabel::AllocNotify() => {}
            }
        });
