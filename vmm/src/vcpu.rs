...
use common::{
    config::{
        CNODE_RADIX_BITS, DEFAULT_PARENT_EP, DEFAULT_SERVE_EP, DEFAULT_THREAD_NOTIFICATION,
        PAGE_SIZE,
    },
    page::PhysPage,
};
...
use sel4::{
    ...
    cap::{self, Endpoint, Granule, Notification, SmallPage, VCpu},
    debug_println,
...
impl VCpuTask {
    ...
    pub fn with_context(&self, image: &ElfFile) {
        ...
        *user_context.c_param_mut(0) = 0;
        *user_context.c_param_mut(1) = 0;
        *user_context.pc_mut() = image.header.pt2.entry_point();
        *user_context.spsr_mut() = 1 << 2;
...
pub fn build_kernel_thread(
    ...
    let vcpu = OBJ_ALLOCATOR.alloc_vcpu();
    let srv_ep = OBJ_ALLOCATOR.alloc_endpoint();

    vcpu.vcpu_set_tcb(tcb).unwrap();
    sel4::debug_println!("sel4 identity: {}", vcpu.debug_identify());
    ...
    // 补充剩下的页表
    for addr in (0x4008_0000..0x4300_0000).step_by(0x1000) {
        if task.mapped_page.contains_key(&addr) {
            continue;
        }
        let page_cap = PhysPage::new(OBJ_ALLOCATOR.alloc_page());
        task.map_page(addr, page_cap);
    }
...
pub fn run_tasks(tasks: &[VCpuTask]) {
    tasks.iter().for_each(VCpuTask::run)
}
